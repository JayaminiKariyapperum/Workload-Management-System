...
<body>
	...
	<div class="search">
			<form action="UserView.php" method="get">
				<p>
					<input type="text" name="search" id="" placeholder="Type Employee ID" value="<?php echo $search; ?>" required autofocus>
				</p>
			</form>
	</div>
   ...
   <table class="table">
	 ...
	 if ( isset($_GET['search']) ) {
		 ...
		 $query = "SELECT ENo,EmployeeID,FullName,Actor,Department,Position,EmailID,LastLogin FROM usertable WHERE (EmployeeID LIKE '%{$search}%') AND is_deleted=0 ";					
	 } else {
		 $query = "SELECT ENo,EmployeeID,FullName,Actor,Department,Position,EmailID,LastLogin
		 FROM usertable WHERE is_deleted=0";
	 }
     ...
     if (mysqli_num_rows($result) > 0) {
         while($row = mysqli_fetch_assoc($result)) {
             ...
             echo "<td><a href='UserEdit.php?id=" . $row["ENo"] . "'>Edit</a></td>"; // Add link to edit.php with current record ID as parameter
			 echo "<td><a href='UserDelete.php?id=" . $row["ENo"] . "' onclick=\"return confirm('Are you sure?');\">Delete</a></td>"; 
