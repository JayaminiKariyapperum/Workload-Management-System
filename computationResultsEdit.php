...
if(isset($_GET['id'])){
	...
	$query = "SELECT CRN,CourseCode,TheoryPractical,NumberOfStudents,TotalTime
	...
	if($result_set){
		if(mysqli_num_rows($result_set) == 1){
			...
			// record not found
			header('Location: computationResultsView.php?err=record_not_found');
			exit;
		}
	} else {
		// query unsuccessful
		header('Location: computationResultsView.php?err=query_failed');
		exit;
	}
}
...
if(isset($_POST['SubmitSetting'])){
  ...
  $req_fields = array('id','CourseCode','NoOfStudents','radio_button');
...
<head>
	...
	<title>Computation of Course Results Edit</title>
