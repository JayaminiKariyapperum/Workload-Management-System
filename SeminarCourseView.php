...
<head>
	...
	<title>Seminar Course View</title>
...
<body>
   ...
   <table class="table">
     ...
     <tbody>
       ...
       $sql = "SELECT SC_No,CourseCode,TheoryPractical,NumberOfStudents,NumberOfWeeks,TotalTime FROM seminar_course WHERE EmployeeID = '{$_SESSION['EmployeeID']}'  && Academic_year = '{$_SESSION['Academic_year']}' && is_deleted=0";
       ...
       if (mysqli_num_rows($result) > 0) {
         while($row = mysqli_fetch_assoc($result)) {
           ...
           echo "<td>" . $row["NumberOfWeeks"] . "</td>";
           echo "<td>" . $row["TotalTime"] . "</td>";
		   echo "<td><a href='SeminarCourseEdit.php?id=" . $row["SC_No"] . "'>Edit</a></td>"; 
		   echo "<td><a href='SeminarCourseDelete.php?id=" . $row["SC_No"] . "' onclick=\"return confirm('Are you sure?');\">Delete</a></td>"; 
   ...
   <a href="SeminarCourseAdd.php"><button type="button" class="btn btn-primary">Add New</button></a>
   <a href="seminar_download.php"><button type="button" class="btn btn-info">Download</button></a>
 </div>
