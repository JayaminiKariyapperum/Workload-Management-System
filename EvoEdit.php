...
if(isset($_GET['id'])){
	...
	$query = "SELECT EQN,CourseCode,TheoryPractical,NumberOfAss,NumberOfStudents,TotalTime
		FROM evolution_quiz_assignments_tutorials
		WHERE EQN = {$id} LIMIT 1";
	...
	if($result_set){
		if(mysqli_num_rows($result_set) == 1){
			...
			// record not found
			header('Location: EvoView.php?err=record_not_found');
			exit;
		}
	} else {
		// query unsuccessful
		header('Location: EvoView.php?err=query_failed');
		exit;
	}
}
...
if(isset($_POST['SubmitSetting'])){
    ...
    $req_fields = array('CourseCode','NuOfAss','NoOfStudents','radio_button');
...
<?php
    ...
    $ns="";
		...
		if ($_SERVER["REQUEST_METHOD"] == "POST") {
			$ncs = floatval($_POST["NuOfAss"]); //number of credits
            $ns = floatval($_POST["NoOfStudents"]); //number of credits

			$totaltime = round($ncs * $ns * 1)/6; // total time
		}
?>
...
if(isset($_POST['SubmitSettingFinal'])){
  ...
  $req_fields = array('CourseCode','NuOfAss','NoOfStudents','TotalTime','radio_button');
  ...
  if(empty($errors)){
    ...
    $NoOfAss =  mysqli_real_escape_string($connection,$_POST['NuOfAss']);
   ...
   $Theory_Practical = isset($_POST['radio_button']) ? $_POST['radio_button'] : null;
	...
	$query = "UPDATE evolution_quiz_assignments_tutorials SET ";
	$query .= "CourseCode = '{$CourseCode}', ";
	$query .= "NumberOfAss = '{$NoOfAss}', ";
	$query .= "TotalTime = '{$TotalTime}', ";
	$query .= "NumberOfStudents = '{$NoOfStudents}',";
    $query .= "TheoryPractical = '{$Theory_Practical}' ";
	$query .= "WHERE EQN = {$id} LIMIT 1";
  ...
  if($result){
    header('Location: EvoView.php?user_added=true');
  }else{
...
<head>
	...
	<title>Evaluation of Quiz/Assignment Edit</title>
...
<body>
	...
	<div class="breadcrumb">
        ...
        <li><a href="main.php">Home</a></li>
			...
			<li><a href="EvoView.php">Evaluation of Quiz/Assignment</a></li>
			<li>Edit</li>
		</ul>
	</div>
	...
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
		<fieldset>
			...
			Evolution of quiz assignments tutorials</legend>
...
?>
              ...
              <tr>
				...
				<input class="pra" type="radio"  id='radio_button' name="radio_button" value="practical" <?php if(isset($_POST['radio_button']) && $_POST['radio_button'] == 'practical') echo 'checked="checked"'; ?>>Practical
                  ...
                  <tr>
  					<td><label for="NuOfAss">Number of assignment / quiz / tutorials</label></td>
                      <td></td> <td></td> <td></td>
  					<td><input type="text" id="NuOfAss" name="NuOfAss"  <?php echo 'value="' . $noOfAss . '"'; ?> ></td>
                    <td></td>
                    <td>(1/6 hours need for 1 assignment / quiz / tutorials)</td>
                  </tr>
