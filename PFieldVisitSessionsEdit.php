...
if(isset($_GET['id'])){
      ...
      $query = "SELECT FNo, CourseCode, Date, Title, Description, Number_Of_Students, Required_Time
          FROM practical_field_visits
          WHERE FNo = {$id} LIMIT 1";
      ...
      if($result_set){
          if(mysqli_num_rows($result_set) == 1){
              ...
              // record not found
              header('Location: PFieldVisitSessionsView.php?err=record_not_found');
              exit;
          }
      } else {
          // query unsuccessful
          header('Location: PFieldVisitSessionsView.php?err=query_failed');
          exit;
      }
  }
...
if(empty($errors)){
  ...
  $query = "UPDATE practical_field_visits SET ";
  ...
  if($result){
    header('Location: PFieldVisitSessionsView.php?record_modified=true');
  }else{
...
<head>
      ...
      <title>P Fieldvisit Sessions Edit</title>
...
?>
            <form action="PFieldVisitSessionsEdit.php" method="post">
                        <table align="center">
