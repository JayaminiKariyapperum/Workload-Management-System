...
if(isset($_GET['id'])){
	...
	$query = "SELECT MEN,CourseCode,NumberOfCredits,TotalTime
		FROM moderating_exam_papersP
		WHERE MEN = {$id} LIMIT 1";
	...
	if($result_set){
		if(mysqli_num_rows($result_set) == 1){
			...
			// record not found
			header('Location: ModeratingExamPapersViewP.php?err=record_not_found');
			exit;
		}
	} else {
		// query unsuccessful
		header('Location: ModeratingExamPapersViewP.php?err=query_failed');
		exit;
	}
}
...
if(isset($_POST['SubmitSetting'])){
  ...
  $req_fields = array('id','CourseCode','NuOfCredits');
...
<?php
	$ncs="";
		...
		if ($_SERVER["REQUEST_METHOD"] == "POST") {
			...
			$totaltime = round($ncs * 1); // total time
		}
?>
...
$totaltime = $_POST['TotalTime'];
  ...
  $req_fields = array('CourseCode','NuOfCredits','TotalTime');
  ...
  if(empty($errors)){
	...
	$query = "UPDATE moderating_exam_papersP SET ";
	$query .= "CourseCode = '{$CourseCode}', ";
	$query .= "NumberOfCredits = '{$NoOfCredits}', ";
	$query .= "TotalTime = '{$TotalTime}' ";
	$query .= "WHERE MEN = {$id} LIMIT 1";
  ...
  if($result){
    header('Location: ModeratingExamPapersViewP.php?user_added=true');
  }else{
...
<head>
	...
	<title>Moderating Exam Papers Edit P</title>
...
<body>
	...
	<div class="breadcrumb">
        ...
        <li><a href="main.php">Home</a></li>
			...
			<li><a href="ModeratingExamPapersViewP.php">Moderating Exam Papers</a></li>
			<li>Edit</li>
		</ul>
	</div>
...
?>
                  ...
                  <tr>
                      ...
                      <td></td> <td></td> <td></td>
  					<td><input type="text" id="NuOfCredits" name="NuOfCredits"  <?php echo 'value="' . $noOfCredits . '"'; ?> readonly></td>
                    <td></td>
