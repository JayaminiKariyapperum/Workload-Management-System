...
if(isset($_GET['id'])){
	...
	$query = "SELECT Mex,CourseCode,NumberOfCredits,NumberOfStudents,TotalTime
		FROM marking_exam_papers
		WHERE Mex = {$id} LIMIT 1";
	...
	if($result_set){
		if(mysqli_num_rows($result_set) == 1){
			...
			// record not found
			header('Location: markingView.php?err=record_not_found');
			exit;
		}
	} else {
		// query unsuccessful
		header('Location: markingView.php?err=query_failed');
		exit;
	}
}
...
if(empty($errors)){
  ...
  if($result){
    header('Location: markingView.php?user_added=true');
  }else{
...
<head>
	...
	<title>Marking Exam Papers Edit</title>
    ...
    document.getElementById("NuOfCredits").value = credits;
}
	</script>
...
<body>
	...
	<div class="breadcrumb">
        ...
        <li><a href="main.php">Home</a></li>
			...
			<li><a href="MarkingView.php">Marking Exam Papers</a></li>
			<li>Edit</li>
		</ul>
	</div>
...
?>
                  ...
                  <tr>
                    ...
                    <td></td>
                    <td>(2/3 hors need for 1 credit course)</td>
					<script>
