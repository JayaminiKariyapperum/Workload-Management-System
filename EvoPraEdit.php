...
if(isset($_GET['id'])){
	...
	$query = "SELECT EPR,CourseCode,NumberOfStudents,TotalTime
	...
	if($result_set){
		if(mysqli_num_rows($result_set) == 1){
			...
			// record not found
			header('Location: EvoPraView.php?err=record_not_found');
			exit;
		}
	} else {
		// query unsuccessful
		header('Location: EvoPraView.php?err=query_failed');
		exit;
	}
}
...
if(isset($_POST['SubmitSetting'])){
  ...
  $req_fields = array('id','CourseCode','NoOfStudents');
...
$coursecode = $_POST['CourseCode'];
  ...
  if(empty($errors)){
	...
	$query = "UPDATE evolution_practical_reports SET ";
	$query .= "CourseCode = '{$CourseCode}', ";
	$query .= "NumberOfStudents = '{$NoOfStudents}', ";
	$query .= "TotalTime = '{$TotalTime}' ";
	$query .= "WHERE EPR = {$id} LIMIT 1";
...
<head>
	...
	<title>Evaluation of Practical Reports Edit</title>
...
<body>
	...
	<div class="breadcrumb">
        ...
        <li><a href="main.php">Home</a></li>
			...
			<li><a href="EvoPraView.php">Evaluation of Practical reports</a></li>
			<li>Edit</li>			
		</ul>
	</div>
