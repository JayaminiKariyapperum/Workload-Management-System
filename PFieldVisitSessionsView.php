...
<body>
   ...
   <table class="table">
     ...
     $sql = "SELECT FNo, CourseCode, Date, Title, Description, Number_Of_Students, Required_Time
             FROM practical_field_visits
             WHERE EmployeeID = '{$_SESSION['EmployeeID']}'  && Academic_year = '{$_SESSION['Academic_year']}' && is_deleted=0";
     ...
     if (mysqli_num_rows($result) > 0) {
         while($row = mysqli_fetch_assoc($result)) {
             ...
             echo "<td>" . $row["Required_Time"] . "</td>";
             echo "<td><a href='PFieldVisitSessionsEdit.php?id=" . $row["FNo"] . "'>Edit</a></td>"; // Add link to edit.php with current record ID as parameter
             echo "<td><a href='PFieldVisitSessionsDelete.php?id=" . $row["FNo"] . "' onclick=\"return confirm('Are you sure?');\">Delete</a></td>"; 
...
?>
   ...
   <a href="PFieldVisitSessionsAdd.php"><button type="button" class="btn btn-primary">Add New</button></a>
   <a href="field_download.php"><button type="button" class="btn btn-info">Download</button></a>
 </div>
