...
if(isset($_GET['id'])){
	...
	$query = "SELECT TNo,Year,Semester,CourseCode,NumberOfCredits,NumberOfStudents,ActualLectureTime
		FROM academic_instructions_theory_lecture_sessions
		WHERE TNo = {$id} LIMIT 1";
	...
	if($result_set){
		if(mysqli_num_rows($result_set) == 1){
			...
			// record not found
			header('Location: TheorySessionView.php?err=record_not_found');
			exit;
		}
	} else {
		// query unsuccessful
		header('Location: TheorySessionView.php?err=query_failed');
		exit;
	}
}
...
if(isset($_POST['submit1'])){
	...
	$query = "UPDATE academic_instructions_theory_lecture_sessions SET ";
	$query .= "Year = '{$Year}', ";
	...
	$query .= "quiz_ass_tu = '{$quiz_ass_tu}' ";


	
	$query .= "WHERE TNo = {$id} LIMIT 1";
