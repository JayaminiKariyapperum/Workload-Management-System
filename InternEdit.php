...
if(isset($_GET['id'])){
  ...
  $query = "SELECT InNo,RegNo,StudentName,CourseCode,Position,StartDate,EndDate,EvolutionTime
      FROM intern_supervision
      WHERE InNo = {$id} LIMIT 1";
  ...
  if($result_set){
      if(mysqli_num_rows($result_set) == 1){
          ...
          // record not found
          header('Location: InternView.php?err=record_not_found');
          exit;
      }
  } else {
      // query unsuccessful
      header('Location: InternvIEW.php?err=query_failed');
      exit;
  }
}
...
if(empty($errors)){
  ...
  $query = "UPDATE intern_supervision SET ";
  $query .= "RegNo = '{$RegNo}',";
  ...
  $query .= "WHERE InNo = {$id} LIMIT 1";

  $result = mysqli_query($connection,$query);

  if($result){
    header('Location: InternView.php?record_updated=true');
  }else{
...
?>
   <form action="InternEdit.php" method="post">
